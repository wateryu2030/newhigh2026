...
def test_module_imports_fixed():
    ...
    # 修复导入语句
    content = content.replace("from .config import DailyStockConfig", 
    ...
    # 修复导入
    notification_content = notification_content.replace("from .config import DailyStockConfig",
                                                      f"# 动态导入配置\nDailyStockConfig = {config_module.__name__}.DailyStockConfig")
    ...
    # 修复所有导入
    main_content = main_content.replace("from .config import DailyStockConfig",
                                       f"# 动态导入配置\nDailyStockConfig = {config_module.__name__}.DailyStockConfig")
    main_content = main_content.replace("from .data_fetcher import DataFetcher",
                                       f"# 动态导入DataFetcher\nDataFetcher = {fetcher_module.__name__}.DataFetcher")
    main_content = main_content.replace("from .ai_decision import AIDecisionMaker",
                                       f"# 动态导入AIDecisionMaker\nAIDecisionMaker = {ai_module.__name__}.AIDecisionMaker")
    main_content = main_content.replace("from .notification import NotificationSender",
...
async def test_basic_functionality():
    ...
    try:
        ...
        fetcher_content = fetcher_content.replace("from .config import DailyStockConfig",
