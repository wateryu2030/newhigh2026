...
def demo_basic_usage():
    ...
    try:
        ...
        try:
            ...
            # 获取平安银行基本信息
            df = pro.stock_basic(
                ts_code="000001.SZ", fields="ts_code,name,industry,list_date,market"
            )
            if not df.empty:
        ...
        try:
            ...
            ohlcv_list = tushare.fetch_ohlcv(
                code="000001", start_date=start_date, end_date=end_date, period="daily"
            )
            ...
            if ohlcv_list:
                ...
                print(
                    f"  最新数据: {latest.timestamp.date()} 收盘价: {latest.close:.2f} 成交量: {latest.volume:,.0f}"
                )
                print("✓ 日线数据获取成功")
            else:
                print("✗ 未获取到日线数据")
        ...
        try:
            ...
            for stock in stocks:
                try:
                    ohlcv_list = tushare.fetch_ohlcv(
                        code=stock, start_date="20240101", end_date="20240131", period="daily"
                    )
                    if ohlcv_list:
                        latest = ohlcv_list[0]
                        print(
                            f"  {stock}: 1月最新收盘价 {latest.close:.2f}, 成交量 {latest.volume:,.0f}"
                        )
                except Exception as e:
        ...
        try:
            ...
            df = tushare.fetch_financial_data(
                code="000001", report_type="income", start_date="20230101", end_date="20231231"
            )
            if not df.empty:
