...
def check() -> dict:
    ...
    try:
        ...
        out = {}
        for table in ["market_emotion", "sniper_candidates", "trade_signals", "market_signals", "a_stock_limitup"]:
            try:
...
def main() -> int:
    ...
    for k, v in counts.items():
        print(f"  {k}: {v}")
    ok = counts.get("sniper_candidates", -1) >= 0 and (counts.get("market_emotion", 0) > 0 or counts.get("trade_signals", 0) > 0 or counts.get("sniper_candidates", 0) > 0)
    print("Cycle + check done." if ok else "Cycle done; some tables empty (run ensure_market_data first?).")
    return 0
