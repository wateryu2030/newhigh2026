...
- 配置：`prometheus.yml` 抓取 Gateway `http://api:8000/metrics`。
...
3. 导入 Dashboard：可新建 Panel，查询 `http_requests_total` 等（需 Gateway 使用 prometheus_client 暴露指标）；示例 JSON 见 `monitoring/grafana-dashboard-example.json`。

## 告警（可选）

在 `prometheus.yml` 中配置 `alerting` 与 `rule_files`，或使用 Alertmanager。关键指标示例：

- 数据延迟：最后一条 system_status 时间与当前时间差。
- 回撤超限：需业务指标由 Gateway 或 backtest 写入 Prometheus。
