    ...
    try:
        ...
        for _, row in df.iterrows():
            existing = conn.execute(
                "SELECT COUNT(*) FROM news_items WHERE url = ?", 
                [row['url']]
            ).fetchone()[0]
            ...
            if existing == 0:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, [
                    row['symbol'], row['source_site'], row['source'], 
                    row['title'], row['content'], row['url'], row['keyword'],
                    row['tag'], row['publish_time'], row['sentiment_score'], 
                    row['sentiment_label']
                ])
        
        count = conn.execute("SELECT COUNT(*) FROM news_items WHERE source_site = 'caixin.com'").fetchone()[0]
        conn.close()
