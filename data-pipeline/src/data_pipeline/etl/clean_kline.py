...
def clean_kline(code: str | None = None) -> int:
    ...
    if code:
        ...
        """)
    conn.execute("UPDATE a_stock_daily SET volume = COALESCE(NULLIF(volume, NULL), 0) WHERE volume < 0")
    conn.execute("UPDATE a_stock_daily SET amount = COALESCE(NULLIF(amount, NULL), 0) WHERE amount < 0")
