...
def _get_hotmoney_signals() -> list:
    ...
    try:
        ...
        codes = conn.execute(
            "SELECT code, win_rate FROM hotmoney_signals"
        ).fetchdf()
        conn.close()
        out = []
...
class AIFusionStrategy:
    ...
    def generate_signals(
        ...
        if state == "主升":
            candidate_codes = set()
            for code_or_seat, fund_s in hotmoney:
            ...
            if not candidate_codes:
                try:
                    ...
                    if os.path.isfile(get_db_path()):
                        ...
                        conn.close()
                        if ms is not None and not ms.empty:
                            candidate_codes = [str(r.get("code", "")) for _, r in ms.iterrows() if r.get("code")]
                except Exception:
                    pass
    ...
    def save_signals(self, signals: List[Tuple[str, str, float, float, float, float]]) -> int:
        ...
        try:
            ...
            for code, signal, confidence, target_price, stop_loss, signal_score in signals:
                conn.execute(
                    """INSERT INTO trade_signals (code, signal, confidence, target_price, stop_loss, strategy_id, signal_score)
                       VALUES (?, ?, ?, ?, ?, ?, ?)""",
