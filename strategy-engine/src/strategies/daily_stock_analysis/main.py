...
class DailyStockAnalyzer:
    ...
    async def analyze_market(self, markets: Optional[List[str]] = None, 
                           symbols: Optional[Dict[str, List[str]]] = None) -> Dict[str, Any]:
        """
        分析指定市场和股票
        ...
        try:
            ...
            recommendations = await self.ai_decision_maker.generate_recommendations(analysis_results)
    ...
    async def get_recommendations(self, analysis_results: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        获取投资推荐（可基于现有分析结果）
