...
class DataFetcher:
    ...
    async def fetch_market_data(self, markets: List[str], 
                               symbols: Dict[str, List[str]]) -> Dict[str, Any]:
        """
        获取市场数据
    ...
    async def _fetch_market_specific_data(self, market: str, symbols: List[str]) -> Dict[str, Any]:
        ...
        if successful_sources:
            market_data["summary"] = {
                ...
                "data_points": sum(len(data.get("quotes", [])) for data in market_data["data"].values()),
                "status": "partial" if len(successful_sources) < len(self.config.data_sources) else "complete"
            }
        else:
            market_data["summary"] = {
                "data_sources": [],
