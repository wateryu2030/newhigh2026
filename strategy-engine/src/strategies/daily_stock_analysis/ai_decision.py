"""
AI决策模块
集成现有平台的AI服务进行股票分析和决策
支持百炼/通义、Gemini、GPT-4、Claude等AI模型
"""
...
try:
    ...
    dashscope = None
    print("警告: dashscope模块未安装，百炼/通义AI服务不可用")
...
class AIDecisionMaker:
    ...
    def _prepare_qwen_prompt(self, analysis_data: Dict[str, Any]) -> str:
        ...
        prompt_lines = [
            ...
            "1. 分析市场整体情绪和风险水平",
            "2. 推荐3-5只最有投资价值的股票",
            "3. 对每只股票提供：操作建议（买入/持有/卖出）、理由、置信度",
            "4. 提供行业配置建议",
            "5. 给出风险提示",
            "",
            "股票数据：",
    ...
    def _prepare_gemini_prompt(self, analysis_data: Dict[str, Any]) -> str:
        ...
        prompt_lines = [
            ...
            "1. 分析市场整体情绪和风险水平",
            "2. 推荐3-5只最有投资价值的股票",
            "3. 对每只股票提供：操作建议（买入/持有/卖出）、理由、置信度（0-1）",
            "4. 提供行业配置建议",
            "5. 给出风险提示",
            "",
            "请以JSON格式回复，包含以下字段：",
            "- analysis: 包含market_overview, market_sentiment, risk_level, opportunity_areas",
            "- recommendations: 包含top_picks数组（每个元素有symbol, name, action, reason, confidence）",
