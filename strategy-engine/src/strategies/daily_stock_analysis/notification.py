...
class NotificationSender:
    ...
    async def send_analysis_results(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "success_rate": len(successful_channels) / len(self.config.notification_channels) if self.config.notification_channels else 0,
                "detailed_results": send_results,
                "status": "success" if successful_channels else "failed"
            }
            
            self.logger.info(f"通知发送完成: {len(successful_channels)}成功, {len(failed_channels)}失败")
            return summary
    ...
    def _prepare_notification_content(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        content = {
            ...
            "summary": {
                "market_count": analysis_results.get("analysis_data_summary", {}).get("market_count", 0),
                "total_symbols": analysis_results.get("analysis_data_summary", {}).get("total_symbols", 0),
    ...
    async def _send_telegram(self, content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Telegram消息内容: {short_content[:100]}...")
    ...
    async def _send_to_file(self, content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"分析结果已保存到文件: {filename}")
    ...
    async def send_alert(self, alert_type: str, message: str, data: Dict[str, Any] = None) -> Dict[str, Any]:
        """发送警报"""
        self.logger.warning(f"发送警报: {alert_type} - {message}")
        ...
        console_result = await self._send_console({"formatted": {"detailed": f"🚨 警报: {alert_type}\n{message}"}})
