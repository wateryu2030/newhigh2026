...
def test_alpha_score_basic():
    s = alpha_score(sharpe_ratio=1.0, sortino_ratio=1.0, max_drawdown=0.05, win_rate_pct=55, profit_factor=1.5)
    assert 0 <= s <= 1


def test_alpha_score_from_metrics():
    metrics = {"sharpe_ratio": 0.5, "sortino_ratio": 0.6, "max_drawdown": 0.1, "win_rate_pct": 50, "profit_factor": 1.2}
