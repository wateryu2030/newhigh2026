...
1. 一次性：**`cursor agent login`**，或在仓库根 **`.env`** 中设置 **`CURSOR_API_KEY=`**（与 login 二选一；脚本会自动 `source .env`）。未认证时第 2 步会失败并提示。  
1b. 自检：`make check-openclaw-cursor-loop`（检查 CLI + 冒烟生成 `evolution/openclaw_cursor_last_plan.md`）。  
