...
def load_population_from_market(limit: int = 20) -> List[StrategyGene]:
    ...
    try:
        df = conn.execute(
            "SELECT strategy_id, name FROM strategy_market ORDER BY updated_at DESC LIMIT ?", [limit]
        ).fetchdf()
        ...
        for _, row in df.iterrows():
            ...
            if sid:
                genes.append(StrategyGene(rule_tree={}, params={"name": str(row.get("name") or sid)}, strategy_id=sid))
        return genes
    except Exception:
        return []
    finally:
...
def save_gene_to_market(gene: StrategyGene, name: str = "", return_pct: float = None,
    sharpe_ratio: float = None, max_drawdown: float = None, status: str = "active") -> bool:
    try:
