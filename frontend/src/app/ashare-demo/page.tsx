  ...
  return (
    <div className="space-y-8 p-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-white">A股数据Skill演示</h1>
          <p className="text-slate-400 mt-2">基于Tushare的A股数据查询功能演示</p>
        </div>
        <button
          onClick={testAllApis}
          disabled={loading}
          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-white font-medium disabled:opacity-50"
        >
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
          <h3 className="text-red-400 font-medium">错误</h3>
          <p className="text-slate-300 mt-1">{error}</p>
          <p className="text-slate-500 text-sm mt-2">
            请确保Gateway已启动：<code className="bg-slate-800 px-2 py-1 rounded">uvicorn gateway.app:app --host 127.0.0.1 --port 8000</code>
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* 股票基本信息 */}
        <div className="card">
          <h2 className="text-xl font-semibold text-white mb-4">股票基本信息</h2>
          {stockBasic.length > 0 ? (
            <div className="space-y-3">
              {stockBasic.map((stock) => (
                <div key={stock.ts_code} className="p-3 bg-slate-800/50 rounded-lg">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="font-medium text-white">{stock.name}</h3>
                      <p className="text-slate-400 text-sm">{stock.ts_code}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-slate-400 text-sm">行业</p>
        ...
        {/* 财务指标 */}
        <div className="card">
          <h2 className="text-xl font-semibold text-white mb-4">财务指标（贵州茅台）</h2>
          {financeIndicator ? (
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-slate-800/50 rounded-lg">
                <p className="text-slate-400 text-sm">市盈率 (PE)</p>
                <p className="text-2xl font-bold text-white">{financeIndicator.pe}</p>
              </div>
              <div className="p-3 bg-slate-800/50 rounded-lg">
                <p className="text-slate-400 text-sm">市净率 (PB)</p>
                <p className="text-2xl font-bold text-white">{financeIndicator.pb}</p>
              </div>
              <div className="p-3 bg-slate-800/50 rounded-lg">
                <p className="text-slate-400 text-sm">净资产收益率 (ROE)</p>
                <p className="text-2xl font-bold text-white">{financeIndicator.roe}%</p>
              </div>
              <div className="p-3 bg-slate-800/50 rounded-lg">
                <p className="text-slate-400 text-sm">毛利率</p>
                <p className="text-2xl font-bold text-white">{financeIndicator.gross_profit_margin}%</p>
              </div>
              <div className="col-span-2 p-3 bg-slate-800/50 rounded-lg">
                <p className="text-slate-400 text-sm">报告期</p>
                <p className="text-white">{financeIndicator.end_date}</p>
              </div>
            </div>
          ) : (
            <p className="text-slate-500">暂无数据</p>
        ...
        {/* 日线行情 */}
        <div className="card">
          <h2 className="text-xl font-semibold text-white mb-4">日线行情（最近5天）</h2>
          {dailyPrice.length > 0 ? (
                ...
                <tbody>
                  {dailyPrice.map((day, index) => (
                    <tr key={index} className="border-b border-slate-800">
                      <td className="py-2 pr-4 text-slate-300">{day.trade_date}</td>
                      <td className="py-2 pr-4 text-white">{day.open}</td>
                      <td className="py-2 pr-4 text-white">{day.close}</td>
                      <td className={`py-2 pr-4 ${day.pct_chg >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {day.pct_chg >= 0 ? '+' : ''}{day.pct_chg}%
                      </td>
                      <td className="py-2 text-slate-300">{(day.vol / 10000).toFixed(0)}万手</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* 涨停跌停 */}
        <div className="card">
          <h2 className="text-xl font-semibold text-white mb-4">涨停/跌停股票</h2>
              ...
              {limitUpDown.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">{item.ts_code}</p>
                  </div>
                  <div className="text-right">
                    <span className={`px-2 py-1 rounded text-sm ${
                      item.up_down === 'U' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'
                    }`}>
                      {item.up_down === 'U' ? '涨停' : '跌停'}
                    </span>
                    <p className="text-slate-300 text-sm mt-1">{item.close}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* 行业排行 */}
        <div className="card">
          <h2 className="text-xl font-semibold text-white mb-4">行业涨幅排行</h2>
          {industryRanking.length > 0 ? (
            <div className="space-y-2">
              {industryRanking.map((industry, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg">
                  <div>
                    <h3 className="font-medium text-white">{industry.ts_code}</h3>
                    <p className="text-slate-400 text-sm">{industry.trade_date}</p>
                  </div>
                  <div className="text-right">
                    <p className={`text-lg font-bold ${industry.pct_chg >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {industry.pct_chg >= 0 ? '+' : ''}{industry.pct_chg}%
                    </p>
                    <p className="text-slate-400 text-sm">收盘: {industry.close}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* 市场概览 */}
        <div className="card">
          <h2 className="text-xl font-semibold text-white mb-4">市场概览</h2>
          {marketOverview ? (
            <div className="space-y-4">
              <div>
                <h3 className="text-slate-400 text-sm mb-2">主要指数</h3>
                <div className="space-y-2">
                  {marketOverview.indices?.map((index: any, i: number) => (
                    <div key={i} className="flex justify-between items-center p-2 bg-slate-800/50 rounded">
                      <span className="text-white">{index.ts_code}</span>
                      <span className={`${index.pct_chg >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
              ...
              <div>
                <h3 className="text-slate-400 text-sm mb-2">市场统计</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-slate-800/50 rounded">
                    <p className="text-slate-400 text-sm">总市值</p>
                    <p className="text-xl font-bold text-white">{marketOverview.market_stats?.total_market_cap}万亿</p>
                  </div>
                  <div className="p-3 bg-slate-800/50 rounded">
                    <p className="text-slate-400 text-sm">流通市值</p>
                    <p className="text-xl font-bold text-white">{marketOverview.market_stats?.circ_market_cap}万亿</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="card bg-slate-800/80">
        <h2 className="text-lg font-semibold text-white mb-4">API使用说明</h2>
        <div className="space-y-3 text-sm">
          <div>
            <h3 className="text-slate-300 font-medium">可用API端点：</h3>
          ...
          <div>
            <h3 className="text-slate-300 font-medium">启动Gateway：</h3>
            <code className="block bg-slate-900 p-3 rounded mt-2 text-slate-300">
