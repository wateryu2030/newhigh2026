      ...
      <div className="card border-amber-500/20 bg-slate-800/80">
        ...
        {goDataHref && (
          <>
            ...
            <a href="/data" className="mt-3 inline-block text-indigo-400 hover:underline">
              {t('news.goDataPage')} →
            </a>
          </>
        )}
        ...
        <li key={item.url || `${item.title ?? ''}-${item.publish_time ?? ''}-${i}`} className="card list-none">
          ...
          <div className="mt-1 flex flex-wrap gap-2 text-sm text-slate-400">
            ...
            {item.sentiment_label && (
              <span
                className={
                  item.sentiment_score != null && item.sentiment_score < 0 ? 'text-red-400' : 'text-slate-400'
                }
              >
                {item.sentiment_label}
              </span>
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-700 pb-3">
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
            panel === 'market' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
          }`}
        >
          {t('news.tabMarket')}
        </button>
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
            panel === 'collector' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
          }`}
        >
          {t('news.tabCollector')}
        </button>
