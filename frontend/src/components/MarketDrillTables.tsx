...
export function LimitupDrillTable({ rows, dense }: { rows: LimitupDrillItem[]; dense?: boolean }) {
  const { t } = useLang();
...
export function LonghubangDrillTable({ rows, dense }: { rows: LonghubangDrillItem[]; dense?: boolean }) {
  const { t } = useLang();
  const { th, td } = useDenseClass(dense);
  if (!rows.length) return <Empty dense={dense} />;
  return (
    <div className="max-h-[55vh] overflow-auto rounded-lg border border-card-border">
      <table className="w-full min-w-[720px] text-left text-text-primary">
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={`${row.code}-${row.lhb_date}-${i}`} className="border-b border-card-border/40 hover:bg-white/[0.03]">
              <td className={`${td} font-mono`}>{row.code}</td>
              <td className={`${td} max-w-[120px] truncate`} title={row.stock_name}>{row.stock_name?.trim() || '—'}</td>
...
export function FundflowDrillTable({ rows, dense }: { rows: FundflowDrillItem[]; dense?: boolean }) {
  const { t } = useLang();
  const { th, td } = useDenseClass(dense);
  if (!rows.length) return <Empty dense={dense} />;
  return (
